...
/**
 * useEscalations — load + close-out state for the Escalations tab.
 *
 * Reads the authoritative `mining_escalations` ladder through the
 * mounted gateway GET, and drives the acknowledge / resolve closing
 * transitions with an OPTIMISTIC patch + authoritative refetch. The
 * optimistic update is immutable (new array, new row object); a failed
 * call refetches to reconcile, never leaving a torn local state.
 *
 * @module features/central-command/md/escalations/ui/useEscalations
 */
