      ...
      >
        {props.items.map((item) => (
          <figure key={item.id} className="m-0 flex flex-col gap-1">
            ...
            >
              {/* eslint-disable-next-line @next/next/no-img-element -- intentional plain `<img>`: the source URL is dynamic AG-UI media not known at build time, so next/image's required-domain config does not apply */}
              <img
