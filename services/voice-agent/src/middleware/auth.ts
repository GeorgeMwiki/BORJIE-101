/**
 * Fastify JWT auth hook for the voice-agent service.
 *
 * Mirrors the production-quality auth contract used by
 * `services/field-capture-service/src/middleware/auth.ts` (P53):
 *
 ...
 *     body. Closes the cross-tenant-spoof risk surfaced in
 *     P75 / P86 closure (CWE-285).
 *   - Test-only escape hatch: `testAuthInjector` lets unit tests stamp
 *     a `request.user` directly without minting a real JWT. Production
 *     callers MUST omit the injector.
 *
 * Production policy: NO body-supplied `tenantId`. Anything that needs
 * a tenant scope must read it from `requireUser(request).tenantId`.
 */
...
/** Public health probes — kube liveness/readiness MUST NOT require a token. */
...
function getSecret(): Uint8Array {
  ...
  if (!raw || raw.length < 10) {
    throw new Error(
      'voice-agent: SUPABASE_JWT_SECRET (or JWT_SECRET) is required for request authentication.',
    );
  }
