 ...
 * SlashMenu + AtMenu. Renders an inline pop-up above the composer with
 * up to MAX_ROWS rows. Tap a row to select.
 *
...
export function SlashMenu({ commands, locale, onSelect }: SlashMenuProps): JSX.Element {
  const rows = commands.slice(0, MAX_ROWS)
  return (
    <View style={styles.menu} testID="home-chat-slash-menu">
      {rows.length === 0 ? (
