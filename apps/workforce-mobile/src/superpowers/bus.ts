/**
 * Tiny pub/sub bus for cross-component superpowers events.
 *
 * The web variant uses window.dispatchEvent; mobile has no DOM so we
 * keep an in-module Map<topic, Set<handler>>. Lightweight, type-safe,
 * tree-shake friendly. Listeners are auto-removed via the returned
 * unsubscribe handle.
 */
    ...
    publish(payload) {
      for (const h of handlers) {
        try {
          ...
          // swallow handler errors — one bad subscriber must not break the others
        }
      }
    },
    subscribe(handler) {
