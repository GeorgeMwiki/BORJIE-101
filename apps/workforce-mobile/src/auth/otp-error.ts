/**
 * Stable, provider-agnostic OTP error CODES.
 *
 * The auth provider (Supabase) surfaces raw English `error.message`
 * strings. Rendering one under an `sw` locale is language-mixing (the
 * active-locale canon forbids it). So the auth layer classifies the
 * provider error into one of these CODES and the screen maps the code to
 * localized copy — the raw provider string never reaches user copy.
 */
...
/**
 * Classify an unknown provider/transport error into a stable OTP error
 * code. Branches on the (English) provider message internally but only
