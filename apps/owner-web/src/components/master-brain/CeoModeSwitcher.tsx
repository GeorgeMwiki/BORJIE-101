...
interface CeoModeSwitcherProps {
  readonly activeMode: CeoModeId;
  readonly onChange: (mode: CeoModeId) => void;
}

/**
 * 8-mode persona switcher for the Master Brain chat.
 *
 * Controlled — the parent owns mode state so changes propagate to the
 * sibling ChatPanel without remount. Selecting a mode replays its
 * blurb + tool surface + sample prompts so the owner sees what this
 * mode is good for before they type.
 */
export function CeoModeSwitcher({ activeMode, onChange }: CeoModeSwitcherProps) {
  const active = CEO_MODES.find((m) => m.id === activeMode);
      ...
      <div className="mt-5 grid grid-cols-1 gap-5 md:grid-cols-3">
        ...
        <div className="md:col-span-2">
          ...
          <ul className="mt-2 flex flex-col gap-2">
            {active.samplePrompts.map((prompt) => (
              <li
                key={prompt}
