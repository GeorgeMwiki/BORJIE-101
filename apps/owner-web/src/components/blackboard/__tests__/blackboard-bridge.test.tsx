 ...
 * body. The api-gateway strips those tags and re-emits each payload as
 * a typed `board_element` SSE frame. `HomeChatTeach` consumes the
 * frame and calls `appendBoardElement(element, assistantId)`. The
 * mounted `Blackboard` aside subscribes to the module-level store and
 * re-renders with the new element.
 *
 ...
 *  4. Assert the Blackboard re-renders with the element AND the empty
 *     state disappears, proving the chat→board hook fires.
 *
