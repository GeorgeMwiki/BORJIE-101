...
/**
 * O-W-15-NEW — Log new safety incident (client surface).
 *
...
//
// Validation messages are intentionally NOT baked into the schema (which is
// module-scoped and locale-unaware). Field messages are resolved per active
// locale in `handleSubmit` via the `N.validation*` strings (zero-mix canon).
...
/**
 * Resolve a zod validation failure to a message in the ACTIVE locale.
 * The schema itself carries no message strings (it is module-scoped and
 * locale-unaware); the message is chosen here per field so the form never
 * renders an English validation note under a Swahili surface (zero-mix).
 */
function validationMessageFor(
  field: keyof IncidentCreateInput,
  locale: Locale,
): string {
  if (field === 'occurredAt') return pickByLocale(locale, N.validationOccurredAt);
  if (field === 'description') return pickByLocale(locale, N.validationDescription);
  return pickByLocale(locale, N.validationGeneric);
}
...
interface NewIncidentSurfaceProps {
  readonly initialLocale?: Locale;
}

export function NewIncidentSurface({ initialLocale }: NewIncidentSurfaceProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!parsed.success) {
      ...
      for (const issue of parsed.error.issues) {
        ...
        if (key) errs[key] = validationMessageFor(key, locale);
      }
      ...
      {mutation.isError ? (
        <Alert variant="error">
          {mutation.error instanceof ApiError
            ? localizeApiError(mutation.error, locale)
