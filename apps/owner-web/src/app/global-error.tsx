...
/**
 * Owner cockpit root-level error boundary.
 *
 ...
 * and cannot rely on any provider, theme, font, or the i18n runtime
 * (`t()`) from the layout above it — nor on the Tailwind stylesheet,
 * which may not have loaded.
 *
 ...
 * cockpit even when the theme sheet is absent.
 *
 * LOCALE-PURE: the active language is read straight from the
 * `borjie_locale` cookie (`readLocaleFromDocument`) and a SINGLE language
 * is rendered via `pickByLocale` — never a concatenated EN/SW string.
 * The Swahili literals live in the scanner-exempt strings module.
 */
import { useEffect } from 'react';
import { pickByLocale, readLocaleFromDocument } from '@/lib/locale-shared';
import { globalErrorStrings as S } from '@/i18n/strings/global-error';
...
export default function GlobalError({ error, reset }: GlobalErrorProps) {
  useEffect(() => {
    if (process.env.NODE_ENV !== 'production') {
      ...
      console.error('[owner-web/global-error]', error);
    }
  }, [error]);
