...
// zero-mix violation). generateMetadata() runs per request so the template
// reflects the visitor's current cookie.
export async function generateMetadata(): Promise<Metadata> {
  const locale = await readLocaleFromServerCookies();
  const consoleName = pickByLocale(locale, {
    en: 'Borjie Console',
    sw: 'Konsoli ya Borjie',
  });
  const defaultTitle = pickByLocale(locale, {
    en: 'Borjie Console — Internal Admin',
    sw: 'Konsoli ya Borjie — Usimamizi wa Ndani',
  });
