/**
 * Sentry server-side init for admin-web (Node runtime — SSR / RSC /
 * route handlers). Reads `SENTRY_DSN` from the server-only env so the
 * value never lands in the browser bundle.
 *
 * No-op when DSN is unset, so `next dev` without a Sentry project stays
 * silent. Pilot-mode lifts the sample rate to 1.0 to match the client
 * config end-to-end.
 */
