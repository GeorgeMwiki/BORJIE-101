 ...
 *
 * The marketing app does not ship the `@sentry/nextjs` runtime dependency —
 ...
 * we provide this minimal shim so the marketing app's `tsc --noEmit` passes
 * without a runtime dep. If/when `@sentry/nextjs` is added to package.json
 * the real types will take precedence over this declaration.
 *
