 ...
 *
 * Mirrors the LitFin / Mercury three-step pattern: big mono step
 * numerals at the top-left of each cell, then bilingual label kicker,
 * then declarative title, body, and bullets. Cells sit on a single
 * hairline grid so the three steps read as one continuous chain
 * rather than three free-floating cards.
 */
  ...
  return (
    ...
    >
      ...
      <ol className="mt-14 grid gap-px rounded-2xl border border-border bg-border md:grid-cols-3">
        {steps.map((s) => (
          <li key={s.n} className="flex flex-col gap-5 bg-surface p-8">
            <div className="flex items-center justify-between gap-3">
              <span className="font-mono text-5xl font-medium leading-none tracking-tight text-signal-500 tabular-nums">
                {s.n}
              </span>
            ...
            <h3 className="font-display text-xl font-medium leading-tight tracking-tight text-balance">
              {s.title}
            </h3>
