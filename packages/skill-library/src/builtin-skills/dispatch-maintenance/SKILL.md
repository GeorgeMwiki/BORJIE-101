...
description: Triage an inbound plant/equipment maintenance ticket, pick a vendor by category + locality + score, attach SLA based on severity, and write `maintenance_dispatch` to the entity-store.
when_to_use:
  - new plant maintenance ticket created
  - vendor needs assignment
  - severity-3 equipment outage
...
Given a new plant/equipment maintenance ticket and a candidate vendor
list, this skill picks the right vendor by a deterministic scoring rule:
...
- severity 2 (urgent, e.g. crusher down / power loss): respond ≤ 4 hours, resolve ≤ 24 hours.
