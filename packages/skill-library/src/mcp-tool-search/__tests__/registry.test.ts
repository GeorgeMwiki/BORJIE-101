...
describe('McpToolRegistry — search', () => {
  ...
  it('runs under 2s for thousands of tools', () => {
    const reg = new McpToolRegistry();
    ...
    // 2s headroom keeps the perf-regression intent (linear scan over
    // 5000 entries should not blow up super-linearly) while tolerating
    // CI/dev hosts under concurrent vitest load where wall-clock noise
    // can spike a single-shot measurement well past the original 100ms.
