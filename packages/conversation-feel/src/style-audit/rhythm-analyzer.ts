 ...
 * chatbot-rhythm.
 *
 * The analyzer only reads turns and produces a tone hint for the NEXT turn —
 * it never rewrites the current reply, so locale purity is untouched.
 *
 * References:
...
import type { RecentTurn, RhythmScore } from '../types.js';

const PAUSE_SIGNAL_RX =
  /(\bhmm,?\b|\.\.\.|—|--|\bwait,?\b|\blet me check\b|\bngoja\b|\bhebu\b)/i;
 ...
 * pause_signals: count of pause-shaped phrases.
...
export function analyzeRhythm(
  ...
  const assistant = turns.filter((t) => t.role === 'assistant').slice(-windowSize);
