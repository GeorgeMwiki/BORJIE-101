 ...
 *
 * Detects and strips canonical chatbot-feel openers, closers, and
 * apologies before a response leaves the system. Substance is preserved;
 * only filler is removed. Pure — returns a new immutable record.
 *
 * References:
 *  - Anthropic, Sycophancy in Language Models (2024) — filler agreement.
 *  - Strunk + White, "The Elements of Style" — "Omit needless words."
 *
 * Ported from sibling-port src/core/conversation-feel/guards/
 * anti-pattern-stripper.ts onto Borjie's ChatbotFeelPattern taxonomy.
 */
...
/**
 * Pure: strip chatbot-feel patterns while preserving substance.
 */
export function stripChatbotFeel(input: string): StrippedResponse {
  ...
  // Up to 3 passes so chained openers ("Sure! Of course! ...") peel in
  // sequence even though each opener regex anchors to ^.
  ...
  return {
    ...
    residual_chatbot_score: computeResidualScore(stripped),
  };
}
...
function computeResidualScore(text: string): number {
  ...
  if (bullets >= 3 && text.length < 250) score += 15;
  return Math.min(100, score);
}

/**
 * Pure: should stripping escalate to a regen request because the
 * response is now substance-light?
 */
