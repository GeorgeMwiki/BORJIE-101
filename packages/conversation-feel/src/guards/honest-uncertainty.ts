 ...
 *
 * When confidence is below threshold, or required info is missing, or
 * retrieval came back empty, the assistant says "I don't know" directly —
 * no theatrical apology. Format: "I don't know X. [What I do know is Y /
 * I can find out / would you tell me?]". Pure.
 *
 * References:
 *  - Anthropic, Sandbagging in Language Models (2024) — calibrated honesty.
 *  - Lin, Hilton, Evans, "TruthfulQA" (2021).
 *
 * Ported from sibling-port guards/honest-uncertainty.ts; tier thresholds
 * align with Borjie's low/medium/high/critical risk taxonomy.
 */
...
const CONFIDENCE_THRESHOLDS: Record<
  NonNullable<HonestUncertaintyInput["tier"]>,
  number
> = {
  low: 30,
  medium: 45,
...
export function decideHonestUncertainty(
  ...
  if (reason === "missing_info") {
    ...
    line = `I don't have ${topic} in what I can see. ${
      input.known_partial_info
        ? `What I do have: ${input.known_partial_info}.`
        : "Tell me what you know and I'll work from there."
    }`;
  } else {
    line = `I'm not confident on ${topic}. ${
      input.known_partial_info
        ? `What I'm sure of: ${input.known_partial_info}.`
        : "I'd want to verify before saying more."
    }`;
  }
...
/** Pure: strip theatrical apology from a candidate before an "I don't know". */
