/**
 * GitLab salted-hash PII redactor.
 */
...
export const GITLAB_PII_FIELDS: ReadonlyArray<string> = ['email', 'commit_email', 'public_email'];
...
export function createSaltedHashRedactor(deps: SaltedHashRedactorDeps): SaltedHashRedactor {
  const fields = new Set((deps.fields ?? GITLAB_PII_FIELDS).map((f) => f.toLowerCase()));
  return {
