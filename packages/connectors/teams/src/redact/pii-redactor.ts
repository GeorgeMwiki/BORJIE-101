/**
 * Teams salted-hash PII redactor — `mail`, `userPrincipalName`,
 * `email`, phone-like strings in body content.
 *
 * Display names are kept (Mr. Mwikila needs to see attendees).
 */
...
export const TEAMS_PII_FIELDS: ReadonlyArray<string> = [
  'mail',
  'email',
  'userPrincipalName',
  'phoneNumber',
...
export function createSaltedHashRedactor(deps: SaltedHashRedactorDeps): SaltedHashRedactor {
  const fields = new Set((deps.fields ?? TEAMS_PII_FIELDS).map((f) => f.toLowerCase()));
  return {
