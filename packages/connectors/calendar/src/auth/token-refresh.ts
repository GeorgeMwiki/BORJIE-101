/**
 * Calendar token refresh — same shape as the email connector.
 */

import { z } from 'zod';
import type { CalendarProvider, Fetcher } from '../types.js';
...
export function createTokenRefresher(
  ...
  return async (input) => {
    ...
    const url =
      input.provider === 'google_calendar' ? GOOGLE_TOKEN_URL : MICROSOFT_TOKEN_URL;
