/**
 * Linear salted-hash PII redactor.
 *
 * `email`, assignee `email` paths → salted-hash. Issue title/body
 * kept (operational substrate).
 */
...
export const LINEAR_PII_FIELDS: ReadonlyArray<string> = ['email', 'displayEmail'];
...
export function createSaltedHashRedactor(deps: SaltedHashRedactorDeps): SaltedHashRedactor {
  const fields = new Set((deps.fields ?? LINEAR_PII_FIELDS).map((f) => f.toLowerCase()));
  return {
