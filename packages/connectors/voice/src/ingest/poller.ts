/**
 * Twilio Voice poller — lists past calls since the cursor, normalises,
 * advances cursor on `start_time`.
 */

import { listCalls, type ListCallsOutcome } from '../client/voice-client.js';
import { normaliseVoiceCall } from './normalizer.js';
import type { SaltedHashRedactor } from '../redact/pii-redactor.js';
import type { FetcherPort, VoiceCallPayload } from '../types.js';

export interface PollParams {
  readonly authorization: string;
  readonly subAccountSid: string;
  readonly since: string | null;
  readonly pageSize: number;
  readonly redactor: SaltedHashRedactor;
  readonly fetcher: FetcherPort;
  readonly baseUrl?: string;
}

export type PollOutcome =
  | {
      readonly kind: 'ok';
      readonly items: ReadonlyArray<{
        readonly payload: VoiceCallPayload;
        readonly redactionApplied: ReadonlyArray<string>;
        readonly raw: Readonly<Record<string, unknown>>;
      }>;
      readonly nextSince: string;
      readonly hasMore: boolean;
    }
  | { readonly kind: 'auth-failed' }
  | { readonly kind: 'rate-limited'; readonly retryAfterMs: number }
  | { readonly kind: 'upstream-error'; readonly status: number; readonly message: string };

function toYmd(iso: string | null): string | null {
  if (iso === null) return null;
  const ms = Date.parse(iso);
  if (!Number.isFinite(ms)) return null;
  return new Date(ms).toISOString().slice(0, 10);
}

export async function pollVoiceCalls(params: PollParams): Promise<PollOutcome> {
  const outcome: ListCallsOutcome = await listCalls({
    authorization: params.authorization,
    subAccountSid: params.subAccountSid,
    startedAfter: toYmd(params.since),
    pageSize: params.pageSize,
    nextPageUri: null,
    fetcher: params.fetcher,
    ...(params.baseUrl !== undefined ? { baseUrl: params.baseUrl } : {}),
  });
  if (outcome.kind === 'auth-failed') return { kind: 'auth-failed' };
  if (outcome.kind === 'rate-limited') {
    return { kind: 'rate-limited', retryAfterMs: outcome.retryAfterMs };
  }
  if (outcome.kind === 'upstream-error') {
    return { kind: 'upstream-error', status: outcome.status, message: outcome.message };
  }
  const items: Array<{
    readonly payload: VoiceCallPayload;
    readonly redactionApplied: ReadonlyArray<string>;
    readonly raw: Readonly<Record<string, unknown>>;
  }> = [];
  let highest = params.since ?? '';
  for (const call of outcome.calls) {
    const callRec = call as unknown as Readonly<Record<string, unknown>>;
    const { redacted, redactedFields } = await params.redactor.redact(callRec);
    const normalised = normaliseVoiceCall({
      raw: redacted as Readonly<Record<string, unknown>>,
    });
    if (normalised === null) continue;
    items.push({
      payload: normalised,
      redactionApplied: redactedFields,
      raw: redacted as Readonly<Record<string, unknown>>,
    });
    if (normalised.startedAt > highest) highest = normalised.startedAt;
  }
  return {
    kind: 'ok',
    items,
    nextSince: highest === '' ? new Date().toISOString() : highest,
    hasMore: outcome.nextPageUri !== null,
  };
}
