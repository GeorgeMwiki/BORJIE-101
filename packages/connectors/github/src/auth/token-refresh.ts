/**
 * GitHub OAuth refresh (for GitHub Apps with refresh tokens enabled).
 */
...
export async function refreshAccessToken(
  ...
  if (res.status < 200 || res.status >= 300) {
    throw new Error(`GitHub refresh failed: status=${res.status}`);
  }
  const json = JSON.parse(await res.text()) as Record<string, unknown>;
  const access = typeof json.access_token === 'string' ? json.access_token : '';
  if (access === '') throw new Error('GitHub refresh response missing access_token');
