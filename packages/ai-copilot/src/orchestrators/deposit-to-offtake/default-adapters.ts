/**
 * Default adapters for the DepositToOfftakeOrchestrator.
 *
...
export function createDefaultListingPort(
  ...
  return {
    async publishListing(tenantId, siteId, initiatedBy, correlationId) {
      deps.onCall?.('listing.publishListing', { tenantId, siteId, initiatedBy, correlationId });
      // Stable, traceable id; a downstream marketplace adapter can
      // replace this with a real publication id when wired.
      return { listingId: `listing_${tenantId}_${siteId}_${randomUUID().slice(0, 8)}` };
    },
  };
}
...
export function createDefaultInspectionPort(
  ...
  return {
    async scheduleMoveInInspection(tenantId, siteId, customerId) {
      deps.onCall?.('inspection.scheduleMoveInInspection', { tenantId, siteId, customerId });
...
export function createDefaultRenewalPort(
  ...
  return {
    async seedFirstTerm(tenantId, siteId, customerId) {
      deps.onCall?.('renewal.seedFirstTerm', { tenantId, siteId, customerId });
      return { offtakeId: null }; // first lease is created by the lease workflow, not the orchestrator
    },
  };
}
...
export function createDefaultWaitlistPort(
  ...
  return {
    async markUnitFilled(tenantId, siteId) {
      deps.onCall?.('waitlist.markUnitFilled', { tenantId, siteId });
