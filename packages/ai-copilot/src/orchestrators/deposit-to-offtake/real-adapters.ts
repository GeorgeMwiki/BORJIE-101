/**
 * Real adapters for the DepositToOfftakeOrchestrator.
 *
...
import type {
  ...
  DepositToOfftakeOrchestratorDeps,
...
export interface RealNegotiationHints {
   ...
   * orchestrator port carries `listingId` (not `siteId`) so the
...
export interface RealInspectionHints {
  ...
  resolvePropertyId(tenantId: string, siteId: string): Promise<string | null>;
...
export interface RealWaitlistMarkFilled {
  (tenantId: string, siteId: string): Promise<void>;
}
...
export function createRealInspectionAdapter(deps: {
  ...
  return {
    async scheduleMoveInInspection(tenantId, siteId, customerId) {
      const [propertyId, scheduledDate, inspectorId] = await Promise.all([
        deps.hints.resolvePropertyId(tenantId, siteId),
        deps.hints.resolveScheduledDate(tenantId, siteId, customerId),
        deps.hints.resolveInspectorId(tenantId, siteId),
      ]);
      if (!propertyId || !scheduledDate || !inspectorId) {
        return fallback.scheduleMoveInInspection(tenantId, siteId, customerId);
      }
...
export function createRealEventAdapter(deps: {
  ...
  const aggregateType = deps.aggregateType ?? 'OfftakePipelineRun';
 ...
 *
 * Falls back to the default port (returns `{ offtakeId: null }`) when the
 * service returns `null` — typically because the unit is not yet priced
 * or the unit lookup is not wired. The orchestrator's `mobilisation_scheduled`
 * branch is tolerant of a missing offtakeId.
 */
export function createRealRenewalAdapter(deps: {
  ...
  return {
    async seedFirstTerm(tenantId, siteId, customerId) {
      const result = await deps.service.seedFirstTerm({
        tenantId,
        siteId,
        customerId,
      });
      if (!result) return fallback.seedFirstTerm(tenantId, siteId, customerId);
      return { offtakeId: result.offtakeId };
    },
  };
}
...
export function createRealWaitlistAdapter(deps: {
  ...
  if (deps.service) {
    ...
    return {
      async markUnitFilled(tenantId, siteId) {
        await service.markFilled({ tenantId, siteId });
      },
    };
  }
  ...
  return {
    async markUnitFilled(tenantId, siteId) {
      await markFilled(tenantId, siteId);
    },
  };
}
