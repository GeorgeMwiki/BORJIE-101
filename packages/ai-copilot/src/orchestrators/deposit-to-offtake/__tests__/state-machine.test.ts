/**
 * DepositToOfftakeOrchestrator — state-machine + service tests.
 *
 * Covers:
...
import {
  InMemoryOfftakePipelineRunRepository,
  OfftakePipelineError,
  DepositToOfftakeOrchestrator,
...
describe('vacancy-to-lease state machine', () => {
  it('allows the full happy path', () => {
    ...
    expect(
      transition('receiving_inquiries', 'BuyerScreened').nextState,
    ).toBe('screening_buyer');
    expect(
      transition('screening_buyer', 'OfferExtended').nextState,
    ).toBe('offer_extended');
    expect(transition('offer_extended', 'OfferSigned').nextState).toBe(
      'offer_signed',
    );
    expect(
      transition('offer_signed', 'MobilisationScheduled').nextState,
    ).toBe('mobilisation_scheduled');
    expect(
      transition('mobilisation_scheduled', 'OfftakeActivated').nextState,
    ).toBe('offtake_active');
  });

  it('rejects unknown transitions with a diagnostic reason', () => {
    const result = transition('idle', 'OfftakeActivated');
  ...
  it('routes to rejected when applicant fails screening', () => {
    const result = transition('screening_buyer', 'BuyerRejected');
  ...
  it('routes to withdrew when applicant drops mid-offer', () => {
    const result = transition('offer_extended', 'BuyerWithdrew');
  ...
  it('marks offtake_active + rejected + expired + withdrew + cancelled as terminal', () => {
    expect(isTerminal('offtake_active')).toBe(true);
  ...
  it('listAllowedEvents reflects the outgoing edges', () => {
    expect(listAllowedEvents('offer_extended')).toEqual(
      expect.arrayContaining(['OfferSigned', 'OfferExpired', 'BuyerWithdrew']),
    );
    expect(listAllowedEvents('offtake_active')).toEqual([]);
  });
...
function buildTestOrchestrator(
  ...
  readonly orchestrator: DepositToOfftakeOrchestrator;
  readonly repo: InMemoryOfftakePipelineRunRepository;
  ...
  const repo = new InMemoryOfftakePipelineRunRepository();
  const orchestrator = new DepositToOfftakeOrchestrator({
    repo,
    listing,
    enquiry,
    creditRating,
...
describe('DepositToOfftakeOrchestrator service', () => {
  it('startPipeline creates a run and advances to listed', async () => {
    ...
    expect(emittedTypes).toContain('OfftakePipelineStarted');
    expect(emittedTypes).toContain('OfftakePipeline:listed');
  });
  ...
  it('advances through inquiry → screening → offer', async () => {
    ...
    expect(run.buyerCustomerId).toBe('cust_1');
    run = await orchestrator.advance(run, 'BuyerScreened', 'user_admin');
    expect(run.state).toBe('screening_buyer');
  ...
  it('full happy path ends in offtake_active', async () => {
    const { orchestrator } = buildTestOrchestrator();
    let run = await orchestrator.startPipeline({
      tenantId: 't1',
      siteId: 'u1',
    ...
    run = await orchestrator.advance(run, 'MobilisationScheduled', 'user_admin');
    run = await orchestrator.advance(run, 'OfftakeActivated', 'user_admin');
    expect(run.state).toBe('offtake_active');
    expect(run.endedAt).not.toBeNull();
    expect(run.offtakeId).toBe('lease_1');
  });
  ...
  it('advance on a terminal run throws TERMINAL', async () => {
    ...
    ).rejects.toBeInstanceOf(OfftakePipelineError);
  });
