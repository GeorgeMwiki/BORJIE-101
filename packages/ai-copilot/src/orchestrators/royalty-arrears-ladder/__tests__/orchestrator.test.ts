/**
 * RoyaltyArrearsLadderOrchestrator — tests.
 *
...
class InMemoryStore implements RunStorePort {
  ...
  async findRunByOfftake(tenantId: string, offtakeId: string): Promise<RunState | null> {
    for (const r of this.runs.values()) {
      if (r.tenantId === tenantId && r.offtakeId === offtakeId) return this.withSteps(r);
    }
    return null;
  }
...
function buildFixture(
  ...
  const escalationCalls: Array<{ offtakeId: string }> = [];
  const rescindCalls: Array<{ caseId: string }> = [];
  const settlementCalls: Array<{ offtakeId: string }> = [];
  const writeOffCalls: Array<{ offtakeId: string; decision: string }> = [];
  ...
  const notices: NoticeDispatchPort = {
    async sendReminder(input) {
      ...
      return {
        dispatchId: `disp_${input.severity}_${input.offtakeId}`,
  ...
  const escalation: EscalationPort = {
    async escalateToLegal(input) {
      escalationCalls.push({ offtakeId: input.offtakeId });
      return { caseId: `case_${input.offtakeId}` };
    },
  ...
  const settlement: SettlementPort = {
    async createOffer(input) {
      settlementCalls.push({ offtakeId: input.offtakeId });
      return {
        offerId: `off_${input.offtakeId}`,
        discountPct: 10,
        instalments: 3,
      };
    },
  ...
  const writeOff: WriteOffPort = {
    async recordDecision(input) {
      writeOffCalls.push({ offtakeId: input.offtakeId, decision: input.decision });
      return { journalEntryId: `je_${input.offtakeId}` };
    },
  };
  ...
  const logger: RoyaltyArrearsLadderOrchestratorDeps['logger'] = {
    info: () => {},
    warn: () => {},
    error: () => {},
  };

  const orch = new RoyaltyArrearsLadderOrchestrator({
    store,
    notices,
    escalation,
    settlement,
...
describe('RoyaltyArrearsLadderOrchestrator — happy path', () => {
  ...
  it('emits RoyaltyArrearsLadderStarted and RoyaltyArrearsLadderCompleted', async () => {
    const fx = buildFixture({
      autoEscalateOverMinor: 100_000_00,
      autoWriteOffUnderMinor: 300_000_00,
    });
    ...
    expect(types).toContain('RoyaltyArrearsLadderStarted');
    expect(types).toContain('RoyaltyArrearsLadderCompleted');
  });
...
describe('RoyaltyArrearsLadderOrchestrator — compensation', () => {
  it('cancels scheduled reminders when tenant pays midway', async () => {
    ...
    const types = fx.events.map((e) => e.type);
    expect(types).toContain('RoyaltyArrearsLadderCompensated');
  });
});

describe('RoyaltyArrearsLadderOrchestrator — policy gating', () => {
  ...
  it('pauses case_escalation when outstanding is under the threshold', async () => {
    ...
    expect(run.status).toBe('awaiting_approval');
    const gated = run.steps.find((s) => s.stepName === 'case_escalation');
    expect(gated?.policyRule).toBe('royalty.below_auto_escalate_threshold');
  });
  ...
  it('throws when approving a step that is not gated', async () => {
    ...
    ).rejects.toBeInstanceOf(RoyaltyArrearsLadderStepNotGatedError);
  });
});

describe('RoyaltyArrearsLadderOrchestrator — idempotency & retry', () => {
