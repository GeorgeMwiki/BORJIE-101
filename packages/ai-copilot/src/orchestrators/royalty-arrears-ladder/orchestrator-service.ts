/**
 * RoyaltyArrearsLadderOrchestrator — walks an arrears case through the
...
import {
  ROYALTY_ARREARS_LADDER_STEPS,
  type ApproveStepInput,
  type RoyaltyArrearsLadderOrchestratorDeps,
  type Decision,
  type RunState,
  type Step,
  type StepRecord,
...
export class RoyaltyArrearsLadderAlreadyCompletedError extends Error {
  readonly code = 'ROYALTY_ARREARS_LADDER_ALREADY_COMPLETED';
  readonly runId: string;
  constructor(runId: string) {
    super(`Royalty-arrears ladder already completed for this offtake (run ${runId}).`);
    this.runId = runId;
  }
}

export class RoyaltyArrearsLadderRunNotFoundError extends Error {
  readonly code = 'ROYALTY_ARREARS_LADDER_RUN_NOT_FOUND';
}

export class RoyaltyArrearsLadderStepNotGatedError extends Error {
  readonly code = 'ROYALTY_ARREARS_LADDER_STEP_NOT_GATED';
}
...
export class RoyaltyArrearsLadderOrchestrator {
  private readonly deps: RoyaltyArrearsLadderOrchestratorDeps;
  private readonly maxRetries: number;
  private readonly clock: () => Date;
  private readonly idGen: () => string;

  constructor(deps: RoyaltyArrearsLadderOrchestratorDeps) {
    this.deps = deps;
  ...
  async triggerRun(input: TriggerRunInput): Promise<TriggerRunResult> {
    ...
    if (existing) {
      if (existing.status === 'completed' || existing.status === 'compensated') {
        throw new RoyaltyArrearsLadderAlreadyCompletedError(existing.id);
      }
      this.deps.logger.info(
        { runId: existing.id, status: existing.status },
        'royalty-arrears-ladder: resuming existing run',
      );
    ...
    await this.safePublish({
      type: 'RoyaltyArrearsLadderStarted',
      tenantId: input.tenantId,
      runId: run.id,
      payload: {
        offtakeId: input.offtakeId,
  ...
  async getRun(runId: string, tenantId: string): Promise<RunState> {
    ...
    if (!run) throw new RoyaltyArrearsLadderRunNotFoundError(runId);
    return run;
  }
  ...
  async approveStep(input: ApproveStepInput): Promise<RunState> {
    ...
    if (!run) throw new RoyaltyArrearsLadderRunNotFoundError(input.runId);
    ...
    if (!existingStep || existingStep.decision !== 'awaiting_approval') {
      throw new RoyaltyArrearsLadderStepNotGatedError(
    ...
    if (!refreshed) throw new RoyaltyArrearsLadderRunNotFoundError(run.id);
  ...
  private async executeRun(run: RunState): Promise<RunState> {
    ...
    for (let i = 0; i < ROYALTY_ARREARS_LADDER_STEPS.length; i++) {
      const stepName = ROYALTY_ARREARS_LADDER_STEPS[i];
      ...
      // Between-steps compensation check: if the counterparty paid, run
      ...
      try {
        ...
        this.deps.logger.error(
          ...
          'royalty-arrears-ladder: step failed',
        );
      }
      ...
      if (decision === 'awaiting_approval') {
        ...
        await this.safePublish({
          type: 'RoyaltyArrearsLadderAwaitingApproval',
          tenantId: current.tenantId,
          runId: current.id,
    ...
    await this.safePublish({
      type: 'RoyaltyArrearsLadderCompleted',
      tenantId: current.tenantId,
      runId: current.id,
    ...
    while (attempt <= this.maxRetries) {
      try {
        ...
        this.deps.logger.warn(
          ...
          'royalty-arrears-ladder: step retrying',
        );
      }
    }
  ...
  private async stepEscalation(
    ...
    if (run.outstandingMinor < policy.royalty.autoEscalateOverMinor) {
      return {
        decision: 'awaiting_approval',
        actor: 'system',
        policyRule: 'royalty.below_auto_escalate_threshold',
        resultJson: {
          reason: 'Outstanding under auto-escalate threshold — approval required.',
          outstandingMinor: run.outstandingMinor,
          threshold: policy.royalty.autoEscalateOverMinor,
        },
      };
    }
  ...
  private async executeEscalation(
    ...
    return {
      ...
      actor,
      policyRule: 'royalty.auto_escalated',
  ...
  private async stepWriteOffDecision(
    ...
    if (run.outstandingMinor <= policy.royalty.autoWriteOffUnderMinor) {
      return this.executeWriteOff(run, idempotencyKey, 'system', 'write_off');
    }
    return {
      decision: 'awaiting_approval',
      actor: 'system',
      policyRule: 'royalty.write_off_over_threshold',
      resultJson: {
        reason: 'Outstanding over auto-write-off threshold — approval required.',
        outstandingMinor: run.outstandingMinor,
        threshold: policy.royalty.autoWriteOffUnderMinor,
      },
    };
  }
  ...
  private async executeWriteOff(
    ...
    return {
      decision: actor === 'system' ? 'auto_approved' : 'executed',
      actor,
      policyRule: 'royalty.write_off_recorded',
      resultJson: {
        decisionKind,
  ...
  private async compensate(run: RunState): Promise<RunState> {
    ...
    for (const stepName of ROYALTY_ARREARS_LADDER_STEPS) {
        ...
        if (typeof dispatchId === 'string') {
          try {
            ...
            this.deps.logger.warn(
              ...
              'royalty-arrears-ladder: compensation failed (non-fatal)',
            );
          }
        }
      }
    ...
    await this.safePublish({
      type: 'RoyaltyArrearsLadderCompensated',
      tenantId,
      runId: run.id,
  ...
  private async safePublish(event: {
    readonly type:
      | 'RoyaltyArrearsLadderStarted'
      | 'RoyaltyArrearsLadderCompleted'
      | 'RoyaltyArrearsLadderCompensated'
      | 'RoyaltyArrearsLadderAwaitingApproval';
    ...
    try {
      ...
      this.deps.logger.warn(
        ...
        'royalty-arrears-ladder: event publish failed (non-fatal)',
      );
    }
  }
}
