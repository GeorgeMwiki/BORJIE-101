...
function buildFixture(
  ...
  const inspection: InspectionPort = {
    async scheduleInspection(input) {
      return {
        inspectionId: `insp_${input.siteId}`,
  ...
  const deduction: DeductionPort = {
    async compute(input) {
      return { deductionMinor: Math.min(input.damageMinor, input.bondMinor) };
    },
  };

  const refund: RefundPort = {
    async calculate(input) {
      return { refundMinor: Math.max(0, input.bondMinor - input.deductionMinor) };
    },
    async issueRefund(input) {
      refundCalls.push({ refundMinor: input.refundMinor });
      return { refundId: `rfd_${input.offtakeId}`, status: 'IN_TRANSIT' };
    },
  ...
  const autonomy: AutonomyPolicyPort = {
    async getPolicy() {
      return {
        ...
        bondReleases: { autoIssueUnderMinor: opts.autoIssueUnderMinor ?? 1_000_000_00 },
      };
    },
  };
  ...
  const logger: SiteClosureOrchestratorDeps['logger'] = {
    info: () => {},
    warn: () => {},
    error: () => {},
  };

  const orch = new SiteClosureOrchestrator({
    store,
    inspection,
    damageAssessment,
    deduction,
...
describe('SiteClosureOrchestrator — happy path', () => {
  ...
  it('issues refund when positive and emits SiteClosureCompleted', async () => {
    const fx = buildFixture({});
    ...
    expect(fx.events.map((e) => e.type)).toContain('SiteClosureCompleted');
  });
...
describe('SiteClosureOrchestrator — policy gating', () => {
  ...
  it('throws when approving a non-gated step', async () => {
    ...
    ).rejects.toBeInstanceOf(SiteClosureStepNotGatedError);
  });
});

describe('SiteClosureOrchestrator — compensation', () => {
  it('compensates on dispute mid-run', async () => {
    ...
    expect(fx.events.map((e) => e.type)).toContain('SiteClosureDisputed');
  });
...
describe('SiteClosureOrchestrator — retry and idempotency', () => {
