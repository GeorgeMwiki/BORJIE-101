...
function reportOf(id: string, score: number): AssetGradeReport {
  const grade =
  ...
  return {
    ...
    score,
    dimensions: {
      royalty_yield: { dimension: 'royalty_yield', score, grade, explanation: '' },
      opex_efficiency: { dimension: 'opex_efficiency', score, grade, explanation: '' },
      maintenance: { dimension: 'maintenance', score, grade, explanation: '' },
      recovery: { dimension: 'recovery', score, grade, explanation: '' },
      royalty_compliance: { dimension: 'royalty_compliance', score, grade, explanation: '' },
      buyer_quality: { dimension: 'buyer_quality', score, grade, explanation: '' },
    },
    reasons: [],
...
describe('aggregatePortfolioGrade', () => {
  ...
  it('site-count weighting shifts the score toward larger assets', () => {
    const p = aggregatePortfolioGrade('tenant-1', reports, {
      weightBy: 'site_count',
      weightsByAssetId: { A1: 50, B1: 10, C1: 5, D1: 1 },
    });
  ...
  it('asset-value weighting changes the result independently', () => {
    const p = aggregatePortfolioGrade('tenant-1', reports, {
      ...
      weightsByAssetId: { A1: 1, B1: 1, C1: 1, D1: 100 },
    });
  ...
  it('returns top-3 strengths and weaknesses', () => {
    ...
    expect(p.topStrengths.map((r) => r.assetId)).toEqual(['a', 'b', 'c']);
    expect(p.topWeaknesses.map((r) => r.assetId)).toEqual(['e', 'd', 'c']);
  });

  it('INSUFFICIENT_DATA assets bypass the score but are counted in distribution', () => {
    const withMissing: AssetGradeReport[] = [
      reportOf('a', 85),
      {
        ...reportOf('b', 0),
