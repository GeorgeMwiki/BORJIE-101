...
import { AssetGradingService } from '../asset-grading-service.js';
import {
...
function poorInputs(id: string): AssetGradeInputs {
  return {
    assetId: id,
    tenantId: TENANT,
    utilisationRate: 0.4,
    ...
    buyerSatisfactionProxy: 0.35,
    downtimeDays: 90,
    capexDebt: 10_000_000,
    marketPriceRatio: 0.65,
    assetAge: 25,
    siteCount: 8,
  };
}
...
function buildService() {
  ...
  const svc = new AssetGradingService({
...
describe('AssetGradingService.gradeAsset', () => {
  ...
  it('returns INSUFFICIENT_DATA when the asset is unknown', async () => {
    const { svc } = buildService();
    const outcome = await svc.gradeAsset(TENANT, 'ghost');
    expect(outcome.kind).toBe('insufficient');
    if (outcome.kind === 'insufficient') {
      expect(outcome.report.missingFields).toContain('asset_not_found');
    }
  });
  ...
  it('returns INSUFFICIENT_DATA when a required field is NaN', async () => {
    ...
    metrics.seed(TENANT, {
      ...goodInputs('p1'),
      royaltyCollectionRate: Number.NaN,
    });
    const outcome = await svc.gradeAsset(TENANT, 'p1');
    expect(outcome.kind).toBe('insufficient');
    if (outcome.kind === 'insufficient') {
      expect(outcome.report.missingFields).toContain('royaltyCollectionRate');
    }
  });
  ...
  it('persists a snapshot after each successful grade', async () => {
    ...
    metrics.seed(TENANT, goodInputs('p1'));
    await svc.gradeAsset(TENANT, 'p1');
  ...
  it('enforces tenant isolation — grading tenant A does not affect tenant B', async () => {
    const { metrics, svc } = buildService();
    metrics.seed(TENANT, goodInputs('p1'));
    const other = await svc.gradeAsset(OTHER, 'p1');
    expect(other.kind).toBe('insufficient');
  });
});

describe('AssetGradingService.gradeAllAssets', () => {
  it('grades every asset known for the tenant', async () => {
    const { metrics, svc } = buildService();
    metrics.seed(TENANT, goodInputs('a'));
    metrics.seed(TENANT, goodInputs('b'));
    metrics.seed(TENANT, poorInputs('c'));
    const outcomes = await svc.gradeAllAssets(TENANT);
...
describe('AssetGradingService.getPortfolioGrade', () => {
  it('returns a portfolio grade after individual grades have been recorded', async () => {
    ...
    metrics.seedHints(TENANT, {
      siteCountByAssetId: { a: 20, b: 5 },
      assetValueByAssetId: { a: 50_000_000, b: 10_000_000 },
    });
    await svc.gradeAllAssets(TENANT);
    const portfolio = await svc.getPortfolioGrade(TENANT, { weightBy: 'site_count' });
    expect(portfolio.totalAssets).toBe(2);
    // With 20 sites on the good asset and 5 on the poor one, score skews high
...
describe('AssetGradingService.trackOverTime', () => {
...
describe('AssetGradingService.setWeights + getWeights', () => {
  it('accepts and persists custom weights', async () => {
    ...
    expect(updated.royalty_yield).toBe(0.3);
    const fetched = await svc.getWeights(TENANT);
    expect(fetched.royalty_yield).toBe(0.3);
  });
