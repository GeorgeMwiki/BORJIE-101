/**
 * Dynamic price optimizer tests (Agent PhL).
 *
...
import { createDynamicPriceOptimizer } from '../dynamic-pricing/optimizer.js';
import type {
  ApprovalQueuePort,
  PricingInputs,
  PricingLLMPort,
...
function makeRepo(): {
  repo: PriceRecommendationRepository;
  rows: PriceRecommendation[];
} {
  const rows: PriceRecommendation[] = [];
  return {
    rows,
    repo: {
      async insert(row) {
...
function makeInputs(overrides: Partial<PricingInputs> = {}): PricingInputs {
  return {
    ...
    siteId: 'prop_1',
    countryCode: 'TZ',
    currentPriceMinor: 500_000, // 500 TZS (minor units)
    currencyCode: 'TZS',
    market: {
      id: 'mkt_1',
      pitId: 'unit_1',
      currencyCode: 'TZS',
      ourPriceMinor: 500_000,
...
describe('DynamicPriceOptimizer', () => {
  it('happy path: LLM recommendation within cap → stored + queued', async () => {
    ...
    const llm: PricingLLMPort = {
      async propose() {
        return {
          recommendedPriceMinor: 530_000, // 6% bump, within any reasonable cap
          confidence: 0.82,
          explanation:
    ...
    const approvalQueue: ApprovalQueuePort = {
      async queuePriceChange() {
        return { approvalRequestId: 'apr_42' };
      },
    };

    const optimizer = createDynamicPriceOptimizer({
      llm,
      repo,
      approvalQueue,
      priceControl: () => ({ maxIncreasePct: 10, sourceCitation: 'TZ-RCA' }),
    ...
    expect(res.data.recommendation.recommendedPriceMinor).toBe(530_000);
  ...
  it('guardrail: LLM proposal above regulatory cap is clamped', async () => {
    ...
    const llm: PricingLLMPort = {
      async propose() {
        return {
          recommendedPriceMinor: 900_000, // 80% bump — well over the 10% cap
          confidence: 0.9,
    ...
    expect(res.data.recommendation.recommendedPriceMinor).toBe(550_000);
  ...
  it('budget guardrail: ledger.assertWithinBudget throws → BUDGET_EXCEEDED', async () => {
    ...
    const optimizer = createDynamicPriceOptimizer({
      ...
      llm,
      repo,
      priceControl: () => ({ maxIncreasePct: null }),
    });
