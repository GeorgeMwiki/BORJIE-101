/**
 * skill.estate.get_buyer_credit — portable, FICO-scale credit rating.
 *
 * Invoked when an operator asks "how reliable is the Geita off-taker?",
 * "credit rating for buyer X?". Composes `scoreTenantCredit` from the
 ...
 *
 * NOTE: the underlying `CreditRatingInputs` field names (rentToIncomeRatio,
 * avgTenancyMonths, subleaseViolationCount, …) are owned by the
 * `credit-rating` subtree and are passed through verbatim; this wrapper only
 * renames its own public surface to the mining counterparty vocabulary.
 */
...
export const BuyerCreditParamsSchema = z.object({
...
export type BuyerCreditParams = z.infer<typeof BuyerCreditParamsSchema>;
...
const DIMENSION_LABELS: Record<string, string> = {
  ...
  rent_to_income: 'Revenue-to-exposure',
  tenancy_length: 'Relationship length',
...
function paramsToInputs(p: BuyerCreditParams): CreditRatingInputs {
  return {
    tenantId: p.tenantId,
...
export function runBuyerCredit(params: BuyerCreditParams): BuyerCreditResult {
  const parsed = BuyerCreditParamsSchema.parse(params);
...
export const buyerCreditTool: ToolHandler = {
  name: 'skill.estate.get_buyer_credit',
  description:
    'Compute a FICO-scale (300-850) buyer / counterparty credit rating from live payment history. Returns numeric score, letter grade, 5-factor breakdown, and a credit_rating_card block for the blackboard.',
  parameters: {
    type: 'object',
  ...
  async execute(params) {
    const parsed = BuyerCreditParamsSchema.safeParse(params);
    if (!parsed.success) return { ok: false, error: parsed.error.message };
    const result = runBuyerCredit(parsed.data);
