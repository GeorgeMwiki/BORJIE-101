/**
 * royalty_reminder_agent — 3 days before due_date sends SMS/WhatsApp/email
...
export const royaltyReminderAgent: TaskAgent<typeof PayloadSchema> = {
  id: 'royalty_reminder_agent',
  title: 'Royalty Reminder',
  description:
    'Sends royalty-due reminders the configured number of days before the due date.',
  trigger: {
    kind: 'cron',
    cron: '0 7 * * *',
      ...
      for (const channel of ctx.payload.channels) {
        try {
          await dispatcher.dispatch({
            ...
            channel,
            subject: 'Royalty reminder',
            body: `Royalty due ${inv.dueDate} — ${inv.amount}`,
