...
export function renderCertificateDocument(
  ...
  return {
    title: 'BORJIE Tenant Credit Certificate',
    lines: [
      `Issuer: ${p.issuer}`,
      ...
      `Expires: ${p.expiresAt}`,
      scoreLine,
      `Payment history: ${((p.dimensions.payment_history ?? 0) * 100).toFixed(0)}%`,
      `Promise keeping: ${((p.dimensions.promise_keeping ?? 0) * 100).toFixed(0)}%`,
      `Rent-to-income: ${((p.dimensions.rent_to_income ?? 0) * 100).toFixed(0)}%`,
      `Tenancy stability: ${((p.dimensions.tenancy_length ?? 0) * 100).toFixed(0)}%`,
      `Dispute clean-record: ${((p.dimensions.dispute_history ?? 0) * 100).toFixed(0)}%`,
