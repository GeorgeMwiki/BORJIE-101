/**
 * Royalty-arrears-spike detector.
 *
 * Weekly count of operators/buyers with outstanding royalty payments
 * vs the 4-week rolling mean. Flags when this week's count exceeds the
 * mean by a configurable factor (default 1.5x), which is enough to
 * overshoot natural seasonality.
 *
 * Pure function.
 */
...
export function detectRoyaltyArrearsSpike(
  ctx: TickContext,
): ReadonlyArray<AnomalyEvent> {
  const series = ctx.inputs.royaltyArrears;
  ...
  return [
    {
      type: 'anomaly',
      kind: 'royalty-arrears-spike',
      id: `royalty-arrears-spike:${series.tenantId}:${weekKey(latest.weekStartMs)}`,
      tenantId: ctx.tenantId,
      scope: ctx.scope,
      detectedAt: new Date(ctx.nowMs).toISOString(),
      confidence,
      severity,
      headline: `Outstanding royalties jumped to ${latest.arrearsCount} operator(s) this week (vs 4-week mean of ${baselineMean.toFixed(1)}).`,
      evidence: {
