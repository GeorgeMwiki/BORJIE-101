...
import { createRoyaltyChaserSubMd, ROYALTY_CHASER_NAME } from '../index.js';
import {
...
describe('royalty.chaser — full pipeline', () => {
  it('OBSERVE → MAP → REDESIGN → AUTOMATE produces a draft artefact', async () => {
    const sub = createRoyaltyChaserSubMd({ scope: { tenantId: TENANT } });
    ...
    expect(artifact.skillName.startsWith('royalty-chaser.')).toBe(true);
  ...
  it('records outcomes', async () => {
    ...
    expect(sub.name).toBe(ROYALTY_CHASER_NAME);
  });
});
