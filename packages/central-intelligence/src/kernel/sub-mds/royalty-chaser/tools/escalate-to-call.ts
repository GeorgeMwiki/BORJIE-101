 ...
 *
 * Places an outbound voice call to the counterparty via the configured
...
export function renderCallScript(args: {
  ...
  let script: string;
  if (args.language === 'sw') {
    script = `Habari ${args.tenantName}. Hii ni simu kutoka kwa Borjie kwa niaba ya operesheni ya madini. Tunatambua kuwa kuna deni la ${args.currency} ${major} ambalo limechelewa kwa siku ${args.daysOverdue}. Tunapenda kuelewa hali yako na kupanga njia ya kulipa. Tafadhali wasiliana nasi.`;
  } else {
    script = `Hello ${args.tenantName}, this is a courtesy call from Borjie on behalf of the mining operation regarding an outstanding balance of ${args.currency} ${major} which is ${args.daysOverdue} days overdue. We would like to understand your situation and agree on a payment plan. Please call us back.`;
  }
  return Object.freeze({
