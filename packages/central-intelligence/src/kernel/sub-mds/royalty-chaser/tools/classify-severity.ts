 ...
 * critical (45d+). Counterparty history (first-delinquency, repeat,
 * chronic) shifts the bucket up by one level (max critical).
 *
 * The classifier never escalates past `critical`; filing any legal
 * notice is an HQ-tier action outside this sub-MD's tool-belt.
 */
...
export interface ClassifySeverityArgs {
  ...
  /** True if the counterparty has made any partial payment in the
   *  current overdue cycle. Softens the escalation. */
