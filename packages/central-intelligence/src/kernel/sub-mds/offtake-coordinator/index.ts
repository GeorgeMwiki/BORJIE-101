/**
 * offtake.coordinator — public API for the Tier-C sub-MD.
 *
 * Manages the offtake-agreement lifecycle: detect 60-day renewal window
 * → draft renewal with retention forecast → classify buyer termination
...
import { OFFTAKE_COORDINATOR_PERSONA } from './persona.js';
import { redesignLease } from './redesign.js';

export const OFFTAKE_COORDINATOR_NAME = 'offtake.coordinator';

export const OFFTAKE_COORDINATOR_TOOLS = Object.freeze([
...
export interface OfftakeCoordinatorSubMdArgs {
  readonly scope: ScopeFilter;
  readonly recorder?: OutcomeRecorder;
}

export function createOfftakeCoordinatorSubMd(
  args: OfftakeCoordinatorSubMdArgs,
): SubMd {
  const recorder = args.recorder ?? createOutcomeRecorder();
  return Object.freeze({
    name: OFFTAKE_COORDINATOR_NAME,
    persona: OFFTAKE_COORDINATOR_PERSONA,
    scope: args.scope,
    toolBelt: OFFTAKE_COORDINATOR_TOOLS,
    ...
    async recordOutcome(actual: ActualOutcome, predicted: PredictedOutcome): Promise<void> {
      await recorder.record({ subMdName: OFFTAKE_COORDINATOR_NAME, predicted, actual });
    },
  });
}
...
export { OFFTAKE_COORDINATOR_PERSONA } from './persona.js';
