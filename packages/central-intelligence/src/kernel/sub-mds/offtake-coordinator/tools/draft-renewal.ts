 ...
 *
 * Produces a draft offtake-agreement renewal offer for the owner to
 * review. Uses the forecasting-engine retention curve (injected via the
 * `RetentionForecastPort`) to project P(retain) at the proposed price.
 *
...
export interface RetentionForecastPort {
  /** Given a candidate price, return P(retain) in [0,1]. */
  forecast(args: {
...
function renderSubject(lang: 'en' | 'sw' | 'mixed'): string {
  if (lang === 'sw') return 'Pendekezo la kurefusha mkataba wa mauzo';
  return 'Offtake agreement renewal proposal — for your review';
}
...
function renderBody(args: {
  ...
  if (isSw) {
    ...
    lines.push('');
    lines.push(`Tunapendekeza kurefusha mkataba wako wa mauzo (#${args.leaseId}) kwa miezi ${args.termMonths}.`);
    lines.push(`Bei mpya: ${args.currency} ${major}/tani (ongezeko la ${incPct}%).`);
    lines.push('');
    ...
    lines.push('');
    lines.push(`We would like to propose renewing your offtake agreement (#${args.leaseId}) for ${args.termMonths} months.`);
    lines.push(`Proposed price: ${args.currency} ${major}/tonne (${incPct}% adjustment).`);
    lines.push('');
