 ...
 *
 * Generates a buyer-facing reply to a termination request. Owner
...
function renderBody(args: {
  ...
  if (args.kind === 'notice-of-intent') {
    if (sw) {
      ...
      lines.push(`We received your notice of intent for offtake agreement #${args.leaseId}.`);
      if (args.noticeRequestedDate) lines.push(`Date you proposed: ${args.noticeRequestedDate}.`);
      lines.push(`The offtake agreement requires ${args.minNoticeDays} days' notice.`);
    }
