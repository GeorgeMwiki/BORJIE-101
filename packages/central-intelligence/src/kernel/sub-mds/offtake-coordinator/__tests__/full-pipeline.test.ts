...
import { createOfftakeCoordinatorSubMd, OFFTAKE_COORDINATOR_NAME } from '../index.js';
import {
...
describe('offtake.coordinator — full pipeline', () => {
  it('OBSERVE → MAP → REDESIGN → AUTOMATE', async () => {
    const sub = createOfftakeCoordinatorSubMd({ scope: { tenantId: TENANT } });
    ...
    expect(artifact.skillName.startsWith('offtake-coordinator.')).toBe(true);
  });
  ...
  it('name matches', () => {
    const sub = createOfftakeCoordinatorSubMd({ scope: { tenantId: TENANT } });
    expect(sub.name).toBe(OFFTAKE_COORDINATOR_NAME);
  });
});
