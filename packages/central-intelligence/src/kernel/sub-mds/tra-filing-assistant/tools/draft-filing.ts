 ...
 *
 * Produces a TRA-filing-shaped payload for the owner to review. Does NOT
...
export function draftFiling(args: DraftFilingArgs): DraftErritsPayload {
  ...
  const nextStepGuidance = validation.ok
    ? 'Owner reviews the TRA filing payload and approves → MD routes to platform.file_kra_mri (HQ-tier, four-eye).'
