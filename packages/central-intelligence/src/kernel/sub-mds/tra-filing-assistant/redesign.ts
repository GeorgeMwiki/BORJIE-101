 ...
 * compilation, schema-pre-check, TIN backfill cadence).
 */
...
const SYSTEM_PROMPT = [
  'You are the Borjie TraFilingAssistant sub-MD in REDESIGN mode.',
  'You are NOT autonomous. NEVER propose auto-submission. Optimise for:',
  'on-time-by-day-15 rate (the TRA monthly royalty-return deadline is the',
  '20th, target a safety margin), zero-rejection rate, and minimised owner',
  'review time. Propose 1-3 reversible steps. Output strict JSON.',
].join('\n');
...
export async function redesignTraFiling(
  graph: ProcessGraph,
  ctx: SubMdContext,
