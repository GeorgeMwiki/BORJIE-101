...
function makePort(): {
  port: SuspensionNoticeDraftPort;
  deletes: string[];
  responses: Array<DraftSuspensionNoticeOutput>;
} {
  const deletes: string[] = [];
  const responses: DraftSuspensionNoticeOutput[] = [];
  let id = 0;
  return {
    deletes,
    responses,
    port: {
      async draftNotice(args) {
        id += 1;
        const out: DraftSuspensionNoticeOutput = {
          draftId: `draft-${id}`,
          tenantId: args.tenantId,
          siteId: args.siteId,
          operatorId: args.operatorId,
          breachKind: args.breachKind,
          bodyMarkdown: `# Notice draft for site ${args.siteId}\n\n${args.breachSummary}`,
...
const VALID_INPUT = {
  ...
  breachKind: 'outstanding' as const,
  breachSummary: 'Royalty unpaid for 45 days; multiple reminders ignored.',
};

describe('owner.draft_suspension_notice', () => {
  it('happy path — drafts a non-binding notice for in-scope tenant', async () => {
    const { port } = makePort();
    const tool = createDraftSuspensionNoticeTool({ notices: port });
    ...
    expect(out.output.bodyMarkdown).toContain('site-1');
  });
  ...
  it('refuses when port returns non-draft status (INVARIANT_VIOLATION)', async () => {
    const port: SuspensionNoticeDraftPort = {
      async draftNotice(args) {
        return {
          draftId: 'd-1',
          tenantId: args.tenantId,
  ...
  it('emits OTel span tagged mutate-tier', async () => {
    ...
    expect(otel.spans[0]?.name).toBe('tool.owner.draft_suspension_notice');
