import type { AssetComparisonTableBlock } from '../types';
import type { Language, Translator } from '../../chat-modes/types';

interface Props {
  readonly block: AssetComparisonTableBlock;
  readonly language: Language;
  readonly t?: Translator | undefined;
}

export function AssetComparisonTable({ block, language: _language, t: _t }: Props) {
  const columns = block.columns ?? [];
  const rows = block.rows ?? [];
  return (
    <div
      data-testid="asset-comparison-table"
      style={{
        background: '#fff',
    ...
    >
      ...
      {columns.length === 0 || rows.length === 0 ? (
        <div data-testid="asset-comparison-empty" style={{ fontSize: 12, color: '#94a3b8', fontStyle: 'italic' }}>
          No data
        </div>
      ) : (
