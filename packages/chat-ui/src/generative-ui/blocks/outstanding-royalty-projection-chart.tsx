import type { OutstandingRoyaltyProjectionChartBlock } from '../types';
import type { Language, Translator } from '../../chat-modes/types';

interface Props {
  readonly block: OutstandingRoyaltyProjectionChartBlock;
  readonly language: Language;
  readonly t?: Translator | undefined;
}

export function OutstandingRoyaltyProjectionChart({ block, language: _language, t: _t }: Props) {
  ...
  const title = block.title || 'Outstanding-royalty projection';
  const caption = `${block.monthsDelinquent} months at ${block.monthlyRoyalty.toLocaleString()} ${block.currency}/mo + ${block.lateFeePerMonth.toLocaleString()} ${block.currency} late fee`;
  ...
  return (
    <div
      data-testid="outstanding-royalty-projection-chart"
      style={{
        background: '#fff',
    ...
    >
      ...
      <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', height: 'auto' }} role="img" aria-label={title}>
        ...
        <polyline
          data-testid="outstanding-royalty-line"
          fill="none"
          stroke="#dc2626"
          strokeWidth={2}
          points={points
