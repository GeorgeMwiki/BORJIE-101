import type { OfftakeTimelineDiagramBlock } from '../types';
import type { Language, Translator } from '../../chat-modes/types';

interface Props {
  readonly block: OfftakeTimelineDiagramBlock;
...
export function OfftakeTimelineDiagram({ block, language: _language, t: _t }: Props) {
  const events = block.events ?? [];
  return (
    <div
      data-testid="offtake-timeline-diagram"
      style={{
        background: '#fff',
    ...
    >
      ...
      {events.length === 0 ? (
        <div data-testid="offtake-timeline-empty" style={{ fontSize: 12, color: '#94a3b8', fontStyle: 'italic' }}>
          No offtake events
        </div>
      ) : (
        <ol style={{ listStyle: 'none', padding: 0, margin: 0, position: 'relative' }}>
          {events.map((e, idx) => {
            ...
            return (
              <li
                key={idx}
                data-testid={`offtake-event-${e.status}`}
                style={{
                  display: 'flex',
                  gap: 12,
