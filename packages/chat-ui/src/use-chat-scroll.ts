...
/**
 * useChatScroll — the "stick to the bottom unless the user scrolled up"
 * contract for a streaming transcript. Shared across every Borjie chat
 * surface (marketing widget, owner cockpit, admin) so the anchor law is
 * implemented once.
 *
 * Promoted from apps/owner-web's home-chat `use-scroll-anchor`. The naive
 * effect that keyed on `[messages.length, isStreaming]` never fired DURING a
 * stream, so a tall answer scrolled its own tail off-screen (the "flow breaks
 * midway" report). This hook instead watches the scroll container's content
 * for growth (ResizeObserver + MutationObserver) and re-anchors to the bottom
 * on every growth tick — but ONLY when the user is already near the bottom
 * (gap <= threshold). The instant the user scrolls up past the threshold,
 * auto-follow disengages and `showJumpPill` flips true so a floating
 * "Jump to latest" control can appear.
 *
 * `resetAtStreamStart()` re-engages auto-follow at the top of each new turn so
 * a prior scroll-up doesn't strand the user away from the new answer.
 *
...
export interface ChatScrollAnchor {
  ...
  /** True when the user scrolled up and the latest content is below the fold. */
...
export function useChatScroll(): ChatScrollAnchor {
  ...
  // Track manual scroll: disengage follow when the user scrolls up past the
  ...
  // the user is currently following the bottom.
  useEffect(() => {
    const el = scrollRef.current;
    if (!el) return;
