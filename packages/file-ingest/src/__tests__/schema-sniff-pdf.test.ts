...
describe('schema-sniff: PDF text', () => {
  it('detects whitespace-aligned tabular block from a contract scan', () => {
    const text = readFileSync(join(FIXTURES, 'offtake-agreement-scan.txt'), 'utf8');
    ...
    // The first column header is "Offtake Reference" (a single header cell
    // containing a space — wide-whitespace splitting preserves intra-cell
    // single spaces, splits on 2+ whitespace).
    expect(table.headers).toContain('Offtake Reference');
  ...
  it('parses pipe-delimited tabular text', () => {
    const text =
      'tin | period | filing_type | amount\n' +
      '109-234-567 | 2024-Q4 | VAT | 4520000\n' +
      '109-234-567 | 2024-Q4 | PAYE | 8120000\n' +
      '112-334-220 | 2024-Q4 | VAT | 2210000\n';
    const table = parsePdfText(text);
    expect(table.headers).toEqual(['tin', 'period', 'filing_type', 'amount']);
  ...
  it('falls back to single-column "text" mode for prose paragraphs', () => {
    const prose =
      'This is an offtake agreement between BORJIE and the counterparty.\n' +
...
describe('OCR shim', () => {
  it('staticOcrProvider routes recovered text through PDF heuristic and tags image_ocr', async () => {
    const text =
      ...
      'MIN-GEI-01  |NyankangaBlock  |Geita\n' +
      'MIN-GEI-02  |NyamulilimaBlk  |Geita\n' +
      'MIN-KAH-01  |BulyanhuluReef  |Kahama\n';
